...
describe 'AllscriptsUnityClient' do
  ...
  describe '.raise_if_options_invalid' do
    context 'when not given :mode' do
      it { expect { subject.send(:raise_if_options_invalid, FactoryGirl.build(:allscripts_unity_client_parameters, mode: nil)) }.to raise_error(ArgumentError) }
    end

    context 'when given mode: :json' do
      it { expect { subject.send(:raise_if_options_invalid, FactoryGirl.build(:allscripts_unity_client_parameters, mode: :json)) }.not_to raise_error }
    end

    context 'when given mode: :soap' do
      it { expect { subject.send(:raise_if_options_invalid, FactoryGirl.build(:allscripts_unity_client_parameters, mode: :soap)) }.not_to raise_error }
    end
  end
end