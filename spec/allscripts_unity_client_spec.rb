...
describe 'AllscriptsUnityClient' do
  ...
  describe '.create' do
    context 'when given :mode => :soap' do
      it 'returns a SOAPClient' do
        ...
        expect(allscripts_unity_client.create(parameters).client_type).to be(:soap)
      end
    end
    ...
    context 'when given :mode => :json' do
      it 'returns a JSONClient' do
        ...
        expect(allscripts_unity_client.create(parameters).client_type).to be(:json)
      end
    end
  end
  ...
  describe '.raise_if_parameters_invalid' do
    context 'when not given :base_unity_url' do
      it { expect { allscripts_unity_client.send(:raise_if_parameters_invalid, FactoryGirl.build(:allscripts_unity_client_parameters, :base_unity_url => nil)) }.to raise_error(ArgumentError) }
    end

    context 'when not given :username' do
      it { expect { allscripts_unity_client.send(:raise_if_parameters_invalid, FactoryGirl.build(:allscripts_unity_client_parameters, :username => nil)) }.to raise_error(ArgumentError) }
    end

    context 'when not given :password' do
      it { expect { allscripts_unity_client.send(:raise_if_parameters_invalid, FactoryGirl.build(:allscripts_unity_client_parameters, :password => nil)) }.to raise_error(ArgumentError) }
    end

    context 'when not given :appname' do
      it { expect { allscripts_unity_client.send(:raise_if_parameters_invalid, FactoryGirl.build(:allscripts_unity_client_parameters, :appname => nil)) }.to raise_error(ArgumentError) }
    end
  end
end